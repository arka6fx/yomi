  ...
  },
  {
    ...
    parameters: z
      .object({
        ...
        kernelSlug: z.string().describe("Kernel slug"),
      })
      .passthrough(),
  },
  {
    ...
    parameters: z
      .object({
        id: z.string().describe("Competition ID"),
      ...
      .object({
        ...
        files: z
          .array(
            z
              .object({
                token: z.string().describe("Upload token referencing a prior file upload"),
              })
              .passthrough(),
          )
...
export function makeComposioKaggleDef(executor: ComposioExecutor): ConnectorDef {
  return {
    ...
    icon: "kaggle",
    description:
      "Kaggle — search datasets, inspect dataset/kernel status, and submit to competitions (via Composio).",
    ...
    setup: {
      ...
      collect: [
        ...
        {
          env: "COMPOSIO_KAGGLE_AUTH_CONFIG_ID",
          label: "Composio Kaggle auth config id",
          secret: false,
        },
      ],
