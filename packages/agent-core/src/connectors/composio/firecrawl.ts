...
export const firecrawlComposioSpecs: ComposioToolSpec[] = [
  ...
  },
  {
    ...
    description:
      "Crawl a website starting from a URL, following links up to a depth/page limit. Requires user approval before it runs.",
    parameters: z
      .object({
        url: z.string().describe("Starting URL"),
  ...
  },
  {
    ...
    description:
      "Extract structured data from web pages using a prompt or schema. Requires user approval before it runs.",
    parameters: z
      .object({
        ...
        schema: z
          ...
          .optional()
          .describe("JSON schema for the extracted data"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Extract data",
      preview: `Extract from ${Array.isArray(a["urls"]) ? (a["urls"] as unknown[]).length : 0} URL(s)`,
      confirmText: "Extract",
    }),
  },
  {
...
export function makeComposioFirecrawlDef(executor: ComposioExecutor): ConnectorDef {
  return {
    ...
    icon: "firecrawl",
    description:
      "Firecrawl — scrape, crawl, map, and extract structured data from websites (via Composio).",
    ...
    setup: {
      ...
      collect: [
        ...
        {
          env: "COMPOSIO_FIRECRAWL_AUTH_CONFIG_ID",
          label: "Composio Firecrawl auth config id",
          secret: false,
        },
      ],
