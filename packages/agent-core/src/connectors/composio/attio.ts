...
export const attioComposioSpecs: ComposioToolSpec[] = [
  ...
  },
  {
    ...
    parameters: z.object({ object_id: z.string().describe("Object type ID or slug") }).passthrough(),
  },
  {
    slug: "ATTIO_LIST_RECORDS",
    description: "List records of a specific object type (people, companies, deals, etc.), paginated. Read-only.",
    parameters: z.object({
  ...
  },
  {
    ...
    parameters: z.object({
      ...
      attributes: z.record(z.string(), z.unknown()).optional().describe("Attribute values to search by"),
    }).passthrough(),
  },
  {
  ...
  },
  {
    ...
    description: "Create a new record for a specified object type. Requires user approval before it runs.",
    parameters: z.object({
      object_type: z.string().describe("Object type, e.g. 'people', 'companies', 'deals'"),
      values: z.record(z.string(), z.unknown()).describe("Attribute values for the new record"),
    }).passthrough(),
    preview: (a) => ({ title: "Create record", preview: `Create ${String(a["object_type"] ?? "")} record`, confirmText: "Create record" }),
  },
  {
    ...
    preview: (a) => ({ title: "Update record", preview: `Update ${String(a["object_type"] ?? "")} record ${String(a["record_id"] ?? "")}`, confirmText: "Update record" }),
  },
  {
  ...
  },
  {
    ...
    parameters: z.object({ object_type: z.string().describe("Object type"), record_id: z.string().describe("Record ID") }).passthrough(),
    preview: (a) => ({ title: "Delete record", preview: `Delete ${String(a["object_type"] ?? "")} record ${String(a["record_id"] ?? "")} — this cannot be undone`, confirmText: "Delete" }),
  },
  {
    ...
    preview: (a) => ({ title: "Delete note", preview: `Delete note ${String(a["note_id"] ?? "")} — this cannot be undone`, confirmText: "Delete" }),
  },
]
...
export function makeComposioAttioDef(executor: ComposioExecutor): ConnectorDef {
  return {
    ...
    category: "crm",
    icon: "attio",
    description: "Attio — modern CRM for relationship-driven businesses; manage records, notes, and lists across any object type (via Composio).",
    ...
    setup: {
      ...
      collect: [
        ...
        { env: "COMPOSIO_ATTIO_AUTH_CONFIG_ID", label: "Composio Attio auth config id", secret: false },
      ],
