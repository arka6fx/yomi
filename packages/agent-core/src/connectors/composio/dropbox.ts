...
export const dropboxComposioSpecs: ComposioToolSpec[] = [
  {
    ...
    description:
      "Get the current user's Dropbox account info (name, email, storage usage). Read-only.",
  ...
  },
  {
    ...
    parameters: z
      .object({
        ...
        autorename: z.boolean().optional().describe("Auto-rename on conflict"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Create folder",
      preview: `Create folder ${String(a["path"] ?? "")}`,
  ...
  },
  {
    ...
    preview: (a) => ({
      title: "Upload file",
      preview: `Upload to ${String(a["path"] ?? "")}`,
      confirmText: "Upload",
    }),
  },
  {
    ...
    description:
      "Create a file request link so others can send you files. Requires user approval before it runs.",
    parameters: z
      .object({
        title: z.string().describe("File request title"),
        destination: z.string().describe("Folder path to collect files in"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Create file request",
      preview: `Create file request "${String(a["title"] ?? "")}"`,
      confirmText: "Create",
    }),
  },
  {
    ...
    parameters: z
      .object({
        path: z.string().describe("Path for the new Paper doc"),
        content: z.string().describe("HTML or Markdown content"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Create Paper doc",
      preview: `Create Paper doc at ${String(a["path"] ?? "")}`,
      confirmText: "Create",
    }),
  },
  {
    ...
    description:
      "Move or rename a file or folder in Dropbox. Requires user approval before it runs.",
    parameters: z
      .object({
        from_path: z.string().describe("Current path"),
        to_path: z.string().describe("New path"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Move file/folder",
      preview: `Move ${String(a["from_path"] ?? "")} → ${String(a["to_path"] ?? "")}`,
      confirmText: "Move",
    }),
  },
  {
    ...
    preview: (a) => ({
      ...
      preview: `Delete ${String(a["path"] ?? "")} — this cannot be undone`,
      confirmText: "Delete",
    }),
  },
]
  ...
  return {
    ...
    setup: {
      ...
      collect: [
        ...
        {
          env: "COMPOSIO_DROPBOX_AUTH_CONFIG_ID",
          label: "Composio Dropbox auth config id",
          secret: false,
        },
      ],
