...
export const whatsappComposioSpecs: ComposioToolSpec[] = [
  // ── Read ──────────────────────────────────────────────────────
  {
    ...
    description: "List phone numbers on the WhatsApp Business account. Read-only.",
    parameters: z.object({}).passthrough(),
  },
  {
    slug: "WHATSAPP_GET_PHONE_NUMBER",
    description: "Get details of a specific phone number. Read-only.",
    parameters: z.object({ phone_number_id: z.string().describe("Phone number ID") }).passthrough(),
  },
  {
    slug: "WHATSAPP_GET_BUSINESS_PROFILE",
    description: "Get the business profile for a phone number. Read-only.",
    parameters: z.object({ phone_number_id: z.string().describe("Phone number ID") }).passthrough(),
  },
  {
    slug: "WHATSAPP_GET_MESSAGE_TEMPLATES",
    description: "List message templates. Read-only.",
    parameters: z
      .object({ status: z.string().optional().describe("Filter by approval status") })
      .passthrough(),
  },
  {
    slug: "WHATSAPP_GET_TEMPLATE_STATUS",
    description: "Get the approval status of a message template. Read-only.",
    parameters: z.object({ template_id: z.string().describe("Template ID") }).passthrough(),
  },
  {
    slug: "WHATSAPP_GET_MEDIA",
    description: "Get uploaded media info including a temporary download URL. Read-only.",
    parameters: z.object({ media_id: z.string().describe("Media ID") }).passthrough(),
  },
  {
    slug: "WHATSAPP_GET_MEDIA_INFO",
    description: "Get metadata about uploaded media, without a download URL. Read-only.",
    parameters: z.object({ media_id: z.string().describe("Media ID") }).passthrough(),
  },
    ...
    parameters: z
      .object({
        phone_number_id: z
          .string()
          .optional()
          .describe("Sending phone number ID (auto-filled if the account has one number)"),
        to_number: z.string().describe("Recipient number"),
  ...
  },
  {
    ...
    description:
      "Reply to a specific message in a conversation. Requires user approval before it runs.",
    parameters: z
      .object({
        phone_number_id: z
          .string()
        ...
        reply_to_message_id: z.string().describe("Message ID being replied to"),
      })
      .passthrough(),
    resolvedParams: { phone_number_id: { viaSlug: "WHATSAPP_GET_PHONE_NUMBERS", list: true } },
    preview: (a) => ({
  ...
  },
  {
    ...
    description:
      "Send a media message (image/video/document) via URL. Requires user approval before it runs.",
    parameters: z
      .object({
        phone_number_id: z
          .string()
          ...
          .describe("Sending phone number ID (auto-filled if the account has one number)"),
        to_number: z.string().describe("Recipient number"),
        media_type: z.string().describe("'image', 'video', 'document', or 'audio'"),
        link: z.string().describe("Media URL"),
    ...
    preview: (a) => ({
      title: "Send media",
      preview: `Send ${String(a["media_type"] ?? "media")} to ${String(a["to_number"] ?? "")}`,
      confirmText: "Send",
    }),
  },
  {
    ...
    description:
      "Send previously uploaded media by its media ID. Requires user approval before it runs.",
    parameters: z
      .object({
        phone_number_id: z
          .string()
          ...
          .describe("Sending phone number ID (auto-filled if the account has one number)"),
        to_number: z.string().describe("Recipient number"),
        media_id: z.string().describe("Uploaded media ID"),
    ...
    preview: (a) => ({
      title: "Send media",
      preview: `Send media ${String(a["media_id"] ?? "")}`,
      confirmText: "Send",
    }),
  },
  {
    ...
    preview: (a) => ({
      title: "Send location",
      preview: `Send location "${String(a["name"] ?? "")}"`,
      confirmText: "Send",
    }),
  },
  {
    ...
    parameters: z
      .object({
        phone_number_id: z
          ...
          .describe("Sending phone number ID (auto-filled if the account has one number)"),
        to_number: z.string().describe("Recipient number"),
        contacts: z.array(z.record(z.string(), z.unknown())).describe("Contact cards to send"),
      })
      .passthrough(),
    resolvedParams: { phone_number_id: { viaSlug: "WHATSAPP_GET_PHONE_NUMBERS", list: true } },
    preview: (a) => ({
      title: "Send contacts",
      preview: `Send contacts to ${String(a["to_number"] ?? "")}`,
      confirmText: "Send",
    }),
  },
  {
    ...
    parameters: z
      .object({
        phone_number_id: z
          ...
          .describe("Sending phone number ID (auto-filled if the account has one number)"),
        to_number: z.string().describe("Recipient number"),
        body_text: z.string().describe("Message body"),
        buttons: z.array(z.record(z.string(), z.unknown())).describe("Button definitions"),
      })
      .passthrough(),
    resolvedParams: { phone_number_id: { viaSlug: "WHATSAPP_GET_PHONE_NUMBERS", list: true } },
    preview: (a) => ({
      title: "Send buttons",
      preview: String(a["body_text"] ?? "").slice(0, 100),
      confirmText: "Send",
    }),
  },
  {
    ...
    parameters: z
      .object({
        ...
        to_number: z.string().describe("Recipient number"),
        body_text: z.string().describe("Message body"),
        button_text: z.string().describe("List button label"),
        sections: z.array(z.record(z.string(), z.unknown())).describe("List sections/items"),
      })
      .passthrough(),
    resolvedParams: { phone_number_id: { viaSlug: "WHATSAPP_GET_PHONE_NUMBERS", list: true } },
    preview: (a) => ({
  ...
  },
  {
    ...
    preview: (a) => ({
      title: "Send template",
      preview: `Send template "${String(a["template_name"] ?? "")}"`,
      confirmText: "Send",
    }),
  },
  {
    ...
    description:
      "Upload media to WhatsApp servers for later sending. Requires user approval before it runs.",
    parameters: z
      .object({
        phone_number_id: z
          .string()
          .optional()
          .describe("Phone number ID (auto-filled if the account has one number)"),
        media_type: z.string().describe("Media type"),
    ...
    preview: () => ({
      title: "Upload media",
      preview: "Upload media to WhatsApp",
      confirmText: "Upload",
    }),
  },
  {
    ...
    description:
      "Create a new message template (requires Meta approval before use). Requires user approval before it runs.",
    parameters: z
      .object({
    ...
    preview: (a) => ({
      ...
      preview: `Create template "${String(a["name"] ?? "")}"`,
      confirmText: "Create",
    }),
  },
  ...
  // ── Irreversible ──────────────────────────────────────────────
  {
    ...
    preview: (a) => ({
      ...
      preview: `Delete template ${String(a["template_id"] ?? "")} — this cannot be undone`,
      confirmText: "Delete",
    }),
  },
]
...
export function makeComposioWhatsAppDef(executor: ComposioExecutor): ConnectorDef {
  return {
    ...
    icon: "whatsapp",
    description:
      "WhatsApp Business — send messages, media, templates, and interactive messages (via Composio).",
    ...
    setup: {
      ...
      collect: [
        { env: "COMPOSIO_API_KEY", label: "Composio API key", secret: true },
        {
          ...
          label: "Composio WhatsApp auth config id",
          secret: false,
        },
      ],
