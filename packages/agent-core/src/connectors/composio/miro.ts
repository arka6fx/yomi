...
export const miroComposioSpecs: ComposioToolSpec[] = [
  ...
  },
  {
    ...
    parameters: z
      .object({ board_id: z.string().describe("Board ID"), tag_id: z.string().describe("Tag ID") })
      .passthrough(),
  },
  {
  ...
  },
  {
    ...
    preview: (a) => ({
      title: "Update board",
      preview: `Update board ${String(a["board_id"] ?? "")}`,
      confirmText: "Update",
    }),
  },
  {
    ...
    description:
      "Add an app card item (rich preview card) to a board. Requires user approval before it runs.",
    parameters: z
      .object({
        board_id: z.string().describe("Board ID"),
        data: z.record(z.string(), z.unknown()).describe("Card data (title, description, fields)"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Create app card",
      preview: `Add app card to board ${String(a["board_id"] ?? "")}`,
      confirmText: "Add",
    }),
  },
  {
  ...
  },
  {
    ...
    preview: (a) => ({
      ...
      preview: `Delete app card ${String(a["item_id"] ?? "")} — this cannot be undone`,
      confirmText: "Delete",
    }),
  },
  {
    ...
    preview: (a) => ({
      ...
      preview: `Delete document ${String(a["item_id"] ?? "")} — this cannot be undone`,
      confirmText: "Delete",
    }),
  },
]
...
export function makeComposioMiroDef(executor: ComposioExecutor): ConnectorDef {
  return {
    ...
    icon: "miro",
    description:
      "Miro — collaborative whiteboard platform; browse boards, manage members and app cards (via Composio).",
    ...
    setup: {
      ...
      collect: [
        ...
        {
          env: "COMPOSIO_MIRO_AUTH_CONFIG_ID",
          label: "Composio Miro auth config id",
          secret: false,
        },
      ],
