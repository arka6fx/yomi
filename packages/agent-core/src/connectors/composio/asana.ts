...
export const asanaComposioSpecs: ComposioToolSpec[] = [
  // ── Read ──────────────────────────────────────────────────────
  {
    ...
    description: "Get the authenticated user's Asana profile and accessible workspaces. Read-only.",
    parameters: z.object({}).passthrough(),
  },
  {
    slug: "ASANA_GET_MULTIPLE_WORKSPACES",
    description: "List all accessible workspaces. Read-only.",
    parameters: z.object({}).passthrough(),
  },
  {
    slug: "ASANA_GET_MULTIPLE_PROJECTS",
    description: "List projects, optionally filtered by workspace or team. Read-only.",
    parameters: z
      .object({
  ...
  },
  {
    ...
    description: "List tasks, filterable by project, section, or assignee. Read-only.",
    parameters: z
      .object({
        project: z.string().optional().describe("Project GID"),
        section: z.string().optional().describe("Section GID"),
        assignee: z.string().optional().describe("Assignee GID (requires workspace)"),
        workspace: z.string().optional().describe("Workspace GID"),
      })
      .passthrough(),
  },
  {
    ...
    description: "Get full details of a specific task. Read-only.",
    parameters: z.object({ task_gid: z.string().describe("Task GID") }).passthrough(),
  },
  {
    slug: "ASANA_GET_STORIES_FOR_TASK",
    description: "Get comments/activity (stories) for a task. Read-only.",
    parameters: z.object({ task_gid: z.string().describe("Task GID") }).passthrough(),
  },
  {
    slug: "ASANA_GET_TAGS",
    description: "List tags in a workspace. Read-only.",
    parameters: z
      .object({ workspace: z.string().optional().describe("Workspace GID") })
      .passthrough(),
  },
  {
    slug: "ASANA_GET_TAG",
    description: "Get details of a specific tag. Read-only.",
    parameters: z.object({ tag_gid: z.string().describe("Tag GID") }).passthrough(),
  },
  {
    slug: "ASANA_GET_MULTIPLE_USERS",
    description: "List users in a workspace or team. Read-only.",
    parameters: z
      .object({
        workspace: z.string().optional().describe("Workspace GID"),
        team: z.string().optional().describe("Team GID"),
  ...
  },
  {
    ...
    description: "List goals, optionally filtered by team, project, or workspace. Read-only.",
    parameters: z
      .object({
        workspace: z.string().optional().describe("Workspace GID"),
        team: z.string().optional().describe("Team GID"),
  ...
  {
    slug: "ASANA_GET_GOAL",
    description: "Get details of a specific goal. Read-only.",
    parameters: z.object({ goal_gid: z.string().describe("Goal GID") }).passthrough(),
  },
  {
    slug: "ASANA_GET_PORTFOLIOS",
    description: "List portfolios in a workspace. Read-only.",
    parameters: z.object({ workspace: z.string().describe("Workspace GID") }).passthrough(),
  },
  {
    slug: "ASANA_GET_CUSTOM_FIELDS_FOR_WORKSPACE",
    description: "List custom fields defined in a workspace. Read-only.",
    parameters: z.object({ workspace_gid: z.string().describe("Workspace GID") }).passthrough(),
  },
    ...
    parameters: z
      .object({
        data: z
          ...
          .describe("Project fields, must include 'name' plus 'workspace' or 'team'"),
      })
      .passthrough(),
    preview: () => ({
      title: "Create project",
      preview: "Create a new Asana project",
  ...
  },
  {
    ...
    parameters: z
      .object({
        data: z
          .record(z.string(), z.unknown())
          .describe("Task fields, must include 'name' plus 'workspace', 'parent', or 'projects'"),
      })
      .passthrough(),
    preview: () => ({
      title: "Create task",
      preview: "Create a new Asana task",
  ...
  },
  {
    ...
    preview: (a) => ({
      ...
      preview: `Create subtask "${String(a["name"] ?? "")}"`,
  ...
  },
  {
    ...
    preview: (a) => ({
      ...
      preview: `Add task ${String(a["task_gid"] ?? "")} to section ${String(a["section_gid"] ?? "")}`,
      confirmText: "Add",
    }),
  },
  {
  ...
  },
  {
    ...
    parameters: z
      .object({
        task_gid: z.string().describe("Task GID"),
        followers: z.array(z.string()).describe("User GIDs to add as followers"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Add followers",
      preview: `Add followers to task ${String(a["task_gid"] ?? "")}`,
      confirmText: "Add",
    }),
  },
  {
    ...
    parameters: z
      .object({
        ...
        data: z.record(z.string(), z.unknown()).describe("Tag fields (name, color)"),
      })
      .passthrough(),
    preview: () => ({
      title: "Create tag",
      preview: "Create a new tag",
  ...
  },
  {
    ...
    parameters: z
      .object({
        ...
        status_type: z.string().describe("Status type"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Post status update",
      preview: String(a["title"] ?? "").slice(0, 100),
      confirmText: "Post",
    }),
  },
  {
  ...
  },
  {
    ...
    preview: (a) => ({
      ...
      preview: `Update project ${String(a["project_gid"] ?? "")}`,
      confirmText: "Update",
    }),
  },
  ...
  // ── Irreversible ──────────────────────────────────────────────
  {
    ...
    preview: (a) => ({
      title: "Delete task",
      preview: `Delete task ${String(a["task_gid"] ?? "")} — this cannot be undone`,
      confirmText: "Delete",
    }),
  },
  {
    ...
    preview: (a) => ({
      ...
      preview: `Delete project ${String(a["project_gid"] ?? "")} — this cannot be undone`,
      confirmText: "Delete",
    }),
  },
  {
    ...
    preview: (a) => ({
      title: "Delete tag",
      preview: `Delete tag ${String(a["tag_gid"] ?? "")} — this cannot be undone`,
      confirmText: "Delete",
    }),
  },
]
...
export function makeComposioAsanaDef(executor: ComposioExecutor): ConnectorDef {
  return {
    ...
    icon: "asana",
    description:
      "Asana — manage projects, tasks, sections, tags, goals, and status updates (via Composio).",
    ...
    setup: {
      ...
      collect: [
        ...
        {
          env: "COMPOSIO_ASANA_AUTH_CONFIG_ID",
          label: "Composio Asana auth config id",
          secret: false,
        },
      ],
