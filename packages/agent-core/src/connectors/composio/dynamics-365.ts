...
export const dynamics365ComposioSpecs: ComposioToolSpec[] = [
  ...
  },
  {
    ...
    parameters: z
      .object({ filter: z.string().optional().describe("OData filter expression") })
      .passthrough(),
  },
  {
  ...
  },
  {
    ...
    parameters: z
      .object({
        subject: z.string().optional().describe("Lead subject"),
        lastname: z.string().optional().describe("Last name"),
    ...
    preview: (a) => ({
      title: "Create lead",
      preview: `Create lead ${String(a["lastname"] ?? "")} at ${String(a["companyname"] ?? "")}`,
  ...
  },
  {
    ...
    description:
      "Create a new Dynamics 365 case (incident). Requires user approval before it runs.",
    parameters: z.object({ title: z.string().optional().describe("Case title") }).passthrough(),
    preview: (a) => ({
      title: "Create case",
      preview: `Create case "${String(a["title"] ?? "")}"`,
  ...
  },
  {
    ...
    parameters: z
      .object({
        name: z.string().optional().describe("Invoice name"),
        account_id: z.string().optional().describe("Account ID"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Create invoice",
      preview: `Create invoice "${String(a["name"] ?? "")}"`,
  ...
  },
  {
    ...
    parameters: z
      .object({
        name: z.string().optional().describe("Sales order name"),
        account_id: z.string().optional().describe("Account ID"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Create sales order",
      preview: `Create sales order "${String(a["name"] ?? "")}"`,
  ...
  },
  {
    ...
    preview: (a) => ({
      title: "Update lead",
      preview: `Update lead ${String(a["lead_id"] ?? "")}`,
  ...
  },
  {
    ...
    description:
      "Update an existing Dynamics 365 opportunity. Requires user approval before it runs.",
    parameters: z.object({ opportunity_id: z.string().describe("Opportunity ID") }).passthrough(),
    preview: (a) => ({
      title: "Update opportunity",
      preview: `Update opportunity ${String(a["opportunity_id"] ?? "")}`,
  ...
  },
  {
    ...
    preview: (a) => ({
      title: "Update case",
      preview: `Update case ${String(a["case_id"] ?? "")}`,
      confirmText: "Update case",
    }),
  },
  {
    ...
    preview: (a) => ({
      ...
      preview: `Update invoice ${String(a["invoice_id"] ?? "")}`,
  ...
  },
  {
    ...
    description:
      "Update an existing Dynamics 365 sales order. Requires user approval before it runs.",
    parameters: z.object({ salesorder_id: z.string().describe("Sales order ID") }).passthrough(),
    preview: (a) => ({
      title: "Update sales order",
      preview: `Update sales order ${String(a["salesorder_id"] ?? "")}`,
      confirmText: "Update sales order",
    }),
  },
]
...
export function makeComposioDynamics365Def(executor: ComposioExecutor): ConnectorDef {
  return {
    ...
    icon: "dynamics-365",
    description:
      "Microsoft Dynamics 365 — create and update accounts, contacts, leads, opportunities, cases, invoices, and sales orders (via Composio).",
    ...
    setup: {
      ...
      collect: [
        ...
        {
          env: "COMPOSIO_DYNAMICS_365_AUTH_CONFIG_ID",
          label: "Composio Dynamics 365 auth config id",
          secret: false,
        },
      ],
