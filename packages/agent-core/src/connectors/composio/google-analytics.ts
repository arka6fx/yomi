...
export const googleAnalyticsComposioSpecs: ComposioToolSpec[] = [
  ...
  },
  {
    ...
    parameters: z.object({ name: z.string().describe("Account resource name, e.g. 'accounts/123'") }).passthrough(),
  },
  {
  ...
  },
  {
    ...
    parameters: z.object({
      parent: z.string().describe("Property resource name, e.g. 'properties/123'"),
      expandedDataSet: z.record(z.string(), z.unknown()).describe("Expanded data set configuration"),
    }).passthrough(),
    preview: (a) => ({
  ...
  return {
    ...
    setup: {
      ...
      collect: [
        ...
        { env: "COMPOSIO_GOOGLE_ANALYTICS_AUTH_CONFIG_ID", label: "Composio Google Analytics auth config id", secret: false },
      ],
