...
export const googleSearchConsoleComposioSpecs: ComposioToolSpec[] = [
  ...
  },
  {
    ...
    description:
      "Query search analytics data (clicks, impressions, CTR, position) for a site. Read-only.",
    parameters: z
      .object({
        site_url: z.string().describe("Verified site URL"),
        ...
        dimensions: z
          .array(z.string())
          .optional()
          .describe("Dimensions: 'query', 'page', 'country', 'device', 'date'"),
...
export function makeComposioGoogleSearchConsoleDef(executor: ComposioExecutor): ConnectorDef {
  return {
    ...
    description:
      "Google Search Console — monitor and optimize your site's search performance, index coverage, and sitemaps (via Composio).",
    ...
    setup: {
      ...
      collect: [
        ...
        {
          env: "COMPOSIO_GOOGLE_SEARCH_CONSOLE_AUTH_CONFIG_ID",
          label: "Composio Google Search Console auth config id",
          secret: false,
        },
      ],
