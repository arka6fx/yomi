...
export const firefliesComposioSpecs: ComposioToolSpec[] = [
  {
    ...
    description: "List recorded meeting transcripts, optionally filtered by date/host/participant. Read-only.",
    parameters: z.object({
  ...
  },
  {
    ...
    description: "List Bites (highlight clips) — optionally scoped to yourself, your team, or a transcript. Read-only.",
    parameters: z.object({
  ...
  },
  {
    ...
    description: "Fetch AI-app outputs (e.g. summaries, action items) for a meeting/transcript. Read-only.",
    parameters: z.object({
  ...
  },
  {
    ...
    description: "Add the Fireflies bot to an ongoing meeting to start recording. Requires approval.",
    parameters: z.object({
...
export function makeComposioFirefliesDef(executor: ComposioExecutor): ConnectorDef {
  return {
    ...
    category: "meetings",
    icon: "fireflies",
    description: "Fireflies — AI meeting assistant that records, transcribes, and summarizes meetings across Zoom, Google Meet, Teams, and more (via Composio).",
    ...
    setup: {
      ...
      collect: [
        ...
        { env: "COMPOSIO_FIREFLIES_AUTH_CONFIG_ID", label: "Composio Fireflies auth config id", secret: false },
      ],
