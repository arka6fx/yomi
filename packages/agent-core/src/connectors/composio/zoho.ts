...
export const zohoComposioSpecs: ComposioToolSpec[] = [
  {
    ...
    description: "Retrieve records from a Zoho CRM module (e.g. 'Leads', 'Deals', 'Contacts'). Read-only.",
    parameters: z.object({
  ...
  },
  {
    ...
    preview: (a) => ({ title: "Create Zoho record", preview: `Create record(s) in ${String(a["module_api_name"] ?? "")}`, confirmText: "Create" }),
  },
  {
    slug: "ZOHO_UPDATE_ZOHO_RECORD",
    description: "Update existing records in a Zoho CRM module. Requires user approval before it runs.",
    parameters: z.object({
      module_api_name: z.string().describe("Module API name"),
      data: z.array(z.record(z.string(), z.unknown())).describe("Records to update (must include id)"),
    }).passthrough(),
    preview: (a) => ({ title: "Update Zoho record", preview: `Update record(s) in ${String(a["module_api_name"] ?? "")}`, confirmText: "Update" }),
  },
  {
    slug: "ZOHO_UPDATE_RELATED_RECORDS",
    description: "Associate or update relationships between records across modules. Requires user approval before it runs.",
    parameters: z.object({
    ...
    preview: (a) => ({ title: "Update related records", preview: `Link records to ${String(a["module_api_name"] ?? "")} ${String(a["record_id"] ?? "")}`, confirmText: "Update" }),
  },
  {
    ...
    preview: (a) => ({ title: "Create tag", preview: `Create tag "${String(a["name"] ?? "")}"`, confirmText: "Create tag" }),
  },
  {
    slug: "ZOHO_CONVERT_ZOHO_LEAD",
    description: "Convert a lead into a contact, account, and optionally a deal. Requires user approval before it runs.",
    parameters: z.object({
      lead_id: z.string().describe("Lead ID to convert"),
      account_id: z.string().optional().describe("Existing account to attach to, instead of creating one"),
    }).passthrough(),
    preview: (a) => ({ title: "Convert lead", preview: `Convert lead ${String(a["lead_id"] ?? "")}`, confirmText: "Convert" }),
  },
]
...
export function makeComposioZohoDef(executor: ComposioExecutor): ConnectorDef {
  return {
    ...
    category: "crm",
    icon: "zoho",
    description: "Zoho CRM — generic module-based record management: create, update, link, and convert leads/deals/contacts/custom modules (via Composio).",
    ...
    setup: {
      ...
      collect: [
        ...
        { env: "COMPOSIO_ZOHO_AUTH_CONFIG_ID", label: "Composio Zoho auth config id", secret: false },
      ],
