...
export const jiraComposioSpecs: ComposioToolSpec[] = [
  ...
  {
    slug: "JIRA_GET_ISSUE",
    description: "Get a Jira issue by ID or key. Read-only.",
    parameters: z.object({ issue_id_or_key: z.string().describe("Issue ID or key") }).passthrough(),
  },
  {
    slug: "JIRA_SEARCH_ISSUES",
    description: "Search issues with structured filters or raw JQL. Read-only.",
    parameters: z
      .object({
        jql: z.string().optional().describe("Raw JQL query"),
        project_key: z.string().optional().describe("Project key"),
        assignee: z.string().optional().describe("Assignee filter"),
        text_search: z.string().optional().describe("Free-text search"),
      })
      .passthrough(),
  },
  {
    slug: "JIRA_GET_ALL_PROJECTS",
    description: "List visible projects. Read-only.",
    parameters: z
      .object({ query: z.string().optional().describe("Filter by name/key") })
      .passthrough(),
  },
  {
    slug: "JIRA_GET_ISSUE_TYPES",
    description: "List available issue types. Read-only.",
    parameters: z.object({}).passthrough(),
  },
  {
    slug: "JIRA_GET_ALL_STATUSES",
    description: "List available issue statuses. Read-only.",
    parameters: z.object({}).passthrough(),
  },
  {
  ...
  },
  {
    ...
    description: "List users watching an issue. Read-only.",
    parameters: z.object({ issue_id_or_key: z.string().describe("Issue ID or key") }).passthrough(),
  },
  {
    slug: "JIRA_LIST_BOARDS",
    description: "List Jira boards. Read-only.",
    parameters: z
      .object({ projectKeyOrId: z.string().optional().describe("Filter by project") })
      .passthrough(),
  },
  {
    slug: "JIRA_LIST_SPRINTS",
    description: "List sprints on a board. Read-only.",
    parameters: z
      .object({
        board_id: z.number().int().describe("Board ID"),
        state: z.string().optional().describe("Filter by sprint state"),
      })
      .passthrough(),
  },
  {
    slug: "JIRA_GET_PROJECT_VERSIONS",
    description: "List versions for a project. Read-only.",
    parameters: z
      .object({ project_id_or_key: z.string().describe("Project ID or key") })
      .passthrough(),
  },
  {
    slug: "JIRA_FIND_USERS",
    description: "Search for Jira users by name/email to find account IDs. Read-only.",
    parameters: z.object({ query: z.string().optional().describe("Search query") }).passthrough(),
  },
  {
    slug: "JIRA_GET_CURRENT_USER",
    description: "Get the authenticated user's Jira info. Read-only.",
    parameters: z.object({}).passthrough(),
  },
  ...
  // ── Write ────────────────────────────────────────────────────
  {
    ...
    parameters: z
      .object({
        ...
        issue_type: z.string().optional().describe("Issue type, e.g. 'Bug', 'Task'"),
    ...
    preview: (a) => ({
      title: "Create issue",
      preview: `Create "${String(a["summary"] ?? "")}" in ${String(a["project_key"] ?? "")}`,
  ...
  },
  {
    ...
    preview: (a) => ({
      title: "Update issue",
      preview: `Update issue ${String(a["issue_id_or_key"] ?? "")}`,
      confirmText: "Update",
    }),
  },
  {
    ...
    parameters: z
      .object({
        issue_id_or_key: z.string().describe("Issue ID or key"),
        comment: z.string().describe("Comment text"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Add comment",
      preview: String(a["comment"] ?? "").slice(0, 100),
  ...
  },
  {
    ...
    description:
      "Assign an issue to a user, or unassign it. Requires user approval before it runs.",
    parameters: z
      .object({
        issue_id_or_key: z.string().describe("Issue ID or key"),
        account_id: z.string().optional().describe("Assignee account ID"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Assign issue",
      preview: `Assign issue ${String(a["issue_id_or_key"] ?? "")}`,
      confirmText: "Assign",
    }),
  },
  {
    ...
    description:
      "Transition an issue to a different workflow state. Requires user approval before it runs.",
    parameters: z
      .object({
        issue_id_or_key: z.string().describe("Issue ID or key"),
        transition_id_or_name: z.string().describe("Transition ID or name"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Transition issue",
      preview: `Transition ${String(a["issue_id_or_key"] ?? "")}`,
  ...
  },
  {
    ...
    parameters: z
      .object({
        inward_issue_key: z.string().describe("Inward issue key"),
        outward_issue_key: z.string().describe("Outward issue key"),
        link_type: z.string().describe("Link type, e.g. 'Blocks'"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Link issues",
      preview: `Link ${String(a["inward_issue_key"] ?? "")} ↔ ${String(a["outward_issue_key"] ?? "")}`,
      confirmText: "Link",
    }),
  },
  {
    ...
    parameters: z
      .object({
        ...
        origin_board_id: z.number().int().describe("Board ID"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Create sprint",
      preview: `Create sprint "${String(a["name"] ?? "")}"`,
  ...
  },
  {
    ...
    preview: (a) => ({
      ...
      preview: `Attach file to ${String(a["issue_key"] ?? "")}`,
      confirmText: "Attach",
    }),
  },
  ...
  // ── Irreversible ──────────────────────────────────────────────
  {
    ...
    preview: (a) => ({
      title: "Delete issue",
      preview: `Delete issue ${String(a["issue_id_or_key"] ?? "")} — this cannot be undone`,
      confirmText: "Delete",
    }),
  },
  {
    ...
    preview: (a) => ({
      ...
      preview: `Delete comment ${String(a["id"] ?? "")} — this cannot be undone`,
      confirmText: "Delete",
    }),
  },
]
  ...
  return {
    ...
    setup: {
      ...
      collect: [
        ...
        {
          env: "COMPOSIO_JIRA_AUTH_CONFIG_ID",
          label: "Composio Jira auth config id",
          secret: false,
        },
      ],
