...
export const cloudflareComposioSpecs: ComposioToolSpec[] = [
  // ── Read ──────────────────────────────────────────────────────
  {
    ...
    description: "List Cloudflare accounts accessible to the user. Read-only.",
    parameters: z.object({}).passthrough(),
  },
  {
    slug: "CLOUDFLARE_LIST_ZONES",
    description: "List, search, and filter zones (domains). Read-only.",
    parameters: z
      .object({
  ...
  },
  {
    ...
    description: "List firewall rules for a zone. Read-only.",
    parameters: z.object({ zone_id: z.string().describe("Zone ID") }).passthrough(),
  },
  {
    slug: "CLOUDFLARE_LIST_ACCOUNT_MEMBERS",
    description: "List members of an account. Read-only.",
    parameters: z.object({ account_id: z.string().describe("Account ID") }).passthrough(),
  },
  {
    slug: "CLOUDFLARE_GET_LISTS",
    description: "List WAF lists for an account. Read-only.",
    parameters: z.object({ account_id: z.string().describe("Account ID") }).passthrough(),
  },
  {
    slug: "CLOUDFLARE_LIST_MONITORS",
    description: "List load-balancer monitors for an account. Read-only.",
    parameters: z.object({ account_id: z.string().describe("Account ID") }).passthrough(),
  },
  {
    slug: "CLOUDFLARE_LIST_POOLS",
    description: "List load-balancer pools for an account. Read-only.",
    parameters: z.object({ account_id: z.string().describe("Account ID") }).passthrough(),
  },
  ...
  // ── Write ────────────────────────────────────────────────────
  {
    ...
    parameters: z
      .object({
        ...
        account: z.record(z.string(), z.unknown()).optional().describe("Account reference"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Create zone",
      preview: `Add zone "${String(a["name"] ?? "")}"`,
      confirmText: "Add zone",
    }),
  },
  {
    slug: "CLOUDFLARE_UPDATE_ZONE",
    description:
      "Update zone properties (e.g. pause/unpause). Requires user approval before it runs.",
    parameters: z.object({ zone_id: z.string().describe("Zone ID") }).passthrough(),
    preview: (a) => ({
      title: "Update zone",
      preview: `Update zone ${String(a["zone_id"] ?? "")}`,
      confirmText: "Update",
    }),
  },
  {
    ...
    parameters: z
      .object({
        zone_identifier: z.string().describe("Zone ID"),
        type: z.string().describe("Record type, e.g. 'A', 'CNAME'"),
    ...
    preview: (a) => ({
      ...
      preview: `Create ${String(a["type"] ?? "")} record "${String(a["name"] ?? "")}"`,
  ...
  },
  {
    ...
    parameters: z
      .object({
        zone_identifier: z.string().describe("Zone ID"),
        identifier: z.string().describe("Record ID"),
    ...
    preview: (a) => ({
      ...
      preview: `Update record ${String(a["identifier"] ?? "")}`,
      confirmText: "Update",
    }),
  },
  {
    ...
    description:
      "Create a new empty WAF list for an account. Requires user approval before it runs.",
    parameters: z
      .object({
        account_id: z.string().describe("Account ID"),
        name: z.string().describe("List name"),
        kind: z.string().describe("List kind, e.g. 'ip'"),
      })
      .passthrough(),
    preview: (a) => ({
      title: "Create WAF list",
  ...
  },
  {
    ...
    parameters: z
      ...
      .passthrough(),
    preview: (a) => ({
      title: "Update WAF list",
      preview: `Update list ${String(a["list_id"] ?? "")}`,
      confirmText: "Update",
    }),
  },
  ...
  // ── Irreversible ──────────────────────────────────────────────
  {
    ...
    preview: (a) => ({
      title: "Delete zone",
      preview: `Delete zone ${String(a["zone_identifier"] ?? "")} — this cannot be undone`,
      confirmText: "Delete",
    }),
  },
  {
    ...
    preview: (a) => ({
      ...
      preview: `Delete record ${String(a["identifier"] ?? "")} — this cannot be undone`,
      confirmText: "Delete",
    }),
  },
  {
    ...
    parameters: z
      ...
      .passthrough(),
    preview: (a) => ({
      title: "Delete WAF list",
      preview: `Delete list ${String(a["list_id"] ?? "")} — this cannot be undone`,
      confirmText: "Delete",
    }),
  },
]
...
export function makeComposioCloudflareDef(executor: ComposioExecutor): ConnectorDef {
  return {
    ...
    icon: "cloudflare",
    description:
      "Cloudflare — manage zones, DNS records, WAF lists, firewall rules, and load balancing (via Composio).",
    ...
    setup: {
      ...
      collect: [
        ...
        {
          env: "COMPOSIO_CLOUDFLARE_AUTH_CONFIG_ID",
          label: "Composio Cloudflare auth config id",
          secret: false,
        },
      ],
