  ...
  return {
    ...
    "classroom-getAssignment": tool({
      description:
        ...
        "To produce a solution: (1) obey any submission format the teacher states in the description — required cover-page fields (name, roll number, section, university), file naming, and whether they want a written document or a slide deck; these vary per assignment, so read them from the description rather than assuming. A file-name template is literal: keep its separators and order exactly, substituting only the placeholders, so 'Name_RollNumber_PS2' becomes 'ArkaGarai_29_PS2' and never 'ArkaGarai29_PS2'. Fill each field from what is already known (courseName/courseSection from this tool, the student's details from memory) and ask the user only for what is genuinely unavailable — a roll number exists in no Google API, so it must be asked once and then remembered; " +
