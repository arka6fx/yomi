...
class ComposioConnection(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "toolkit", name="composio_connections_user_toolkit_unique"),
        Index("composio_connections_user_idx", "user_id"),
        Index("composio_connections_account_idx", "connected_account_id"),
        Index("composio_connections_status_idx", "status"),
    )

    id: Mapped[uuid.UUID] = uuid_pk()
    user_id: Mapped[str] = mapped_column(ForeignKey("user.id", ondelete="CASCADE"), nullable=False)
    # Composio entity id is namespaced `yomi:<user_id>`; kept on the row so webhook
    # events can be matched back to a user without a join.
    ...
    status: Mapped[str] = mapped_column(String, nullable=False, server_default=text("'INACTIVE'"))
    ...
    last_trigger_event_at: Mapped[datetime | None] = mapped_column(DateTime)
    created_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, server_default=func.now())
