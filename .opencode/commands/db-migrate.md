...
- Any `DROP COLUMN` or `DROP TABLE` — flag explicitly
- Any `ALTER TABLE ... ADD COLUMN ... NOT NULL` without a default — will fail on non-empty tables
...
Describe the current schema state in one paragraph.

## Safety rules

- Never run `db:push` — it bypasses migration history
- Never apply without showing SQL first
