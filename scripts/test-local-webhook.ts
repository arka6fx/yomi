...
// Run with: bun run scripts/test-local-webhook.ts
import { db, users } from "@yomi/db"
import { eq } from "drizzle-orm"
import { createHmac } from "node:crypto"

const TARGET_URL = "http://localhost:3001/api/billing/webhook"
...
async function run() {
  ...
  let testUser = await db.query.users.findFirst({
    where: eq(users.email, "arkagarai292@gmail.com"),
  })
  ...
  // Retrieve user current plan to compare later
  const initialUserRecord = await db.query.users.findFirst({
    where: eq(users.id, testUser.id),
  })
  console.log(`Initial User Status: plan=${initialUserRecord?.plan}, subscriptionStatus=${initialUserRecord?.subscriptionStatus}`)
  ...
  const cancelledUserRecord = await db.query.users.findFirst({
    where: eq(users.id, testUser.id),
  })
