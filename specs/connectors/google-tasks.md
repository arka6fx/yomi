...
- **Due dates are dates, not times.** Google stores `due` as RFC-3339 but ignores
  the time component entirely. `toDue()` normalises a plain `YYYY-MM-DD` for the
  model, and results are trimmed back to a date on the way out. A task that needs
  a *time* is a calendar event, not a task — the `tasks-createTask` description
  routes the model to `calendar-createEvent` for those.
...
- **`@default` is a real Google alias** for the user's primary list, so every tool
  takes an optional `taskListId` and falls back to it — the model never has to
  look up a list id for the common case.
