...
Define the four-layer architecture, IPC contracts, data flows, and port assignments. This is the boundary document — it defines what each layer owns and what it may not own.
...
All communication is HTTP/JSON on `127.0.0.1:3002`. The desktop passes a shared secret (`SIDECAR_SECRET`) as a header.
...
```json
{
  ...
  "elements": [
    { "label": "Messages tab", "bbox": { "x": 120, "y": 45, "width": 100, "height": 30 } },
    { "label": "Unread badge", "bbox": { "x": 210, "y": 48, "width": 20, "height": 20 } }
  ]
}
```
...
- `packages/shared/src/index.ts` — IPC type definitions (SseEvent, FastQueryRequest, AgentQueryRequest)
...
- WebSocket vs SSE for the sidecar IPC: SSE is simpler and sufficient; WebSocket if bidirectional control (pause/cancel) is needed mid-stream.
- Sidecar crash recovery: desktop should restart the sidecar process if health check fails.
