...
- Better Auth table import: foreign keys reference `users.id` — at implementation time, import the autogenerated Drizzle table from Better Auth's schema export (typically `auth.user` or via `@better-auth/drizzle`). Confirm the exact import path when wiring up the adapter in `apps/backend`.
