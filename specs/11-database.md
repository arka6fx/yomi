...
- Better Auth tables: `user`, `session`, `account`, `verification`. User extended with
  `plan`, `subscription_status`, `trial_start_date`, `trial_end_date`,
  `current_period_end`, `dodo_subscription_id`.
- Billing and usage (pure-credit model): `usage_events` (append-only), `credit_accounts`
  (balance + lifetime totals), `credit_grants` (per-batch with expiry), `credit_transactions`
