...
`SIDECAR_SECRET` (the deleted sidecar was their only caller), the
`ScreenImage` / `screenshot_b64` screen-capture request types in `@yomi/shared`,
and the empty `apps/sidecar` directory. The desktop app itself was already
...
share of the maintenance and OAuth-verification burden while the actual assistant
value had consolidated on the backend + Telegram path. Rather than keep a client
we no longer ship, we make the pivot explicit: one surface, fully owned.
...
- The `voice` and `analyze` credit *kinds* are unchanged in code; only prose
...
- The `screenshot`/`screenshots` keys stay in `ai-telemetry`'s
  `BLOCKED_METADATA_KEYS` denylist — that is a privacy backstop, not a
  desktop coupling, and removing them would only ever weaken it.
