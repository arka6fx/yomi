...
Port of apps/api/src/routes/billing.ts. Dodo API calls go to
...
def dodo_auth() -> str:
    ...
    try:
        ...
        raise RuntimeError(
            f"DODO_{config['mode'].upper()}_API_KEY contains a non-ASCII character; "
            "replace the Worker secret with the raw Dodo API key"
        ) from err
