...
import { adjustSystemVolume, adjustSpotifyVolume, playSpotify, sendWhatsAppMessage, type VolumeDirection } from "../tools/system.js"
...
function spotifyPlaybackQuery(text: string): string | null {
  ...
  const playMatch = text.match(/\bplay\s+(.+?)(?:\s+(?:on|in)\s+spotify\b|$)/i)
    ?? text.match(/\bspotify\s+(?:to\s+)?play\s+(.+?)$/i)
  const raw = playMatch?.[1] ?? text.replace(/\b(open|launch|start)\s+spotify\b/ig, "").replace(/\bspotify\b/ig, "")
  const query = raw
    .replace(/\b(to\s+)?play\b/ig, "")
    .replace(/\b(on|in)\s+spotify\b/ig, "")
    .replace(/[.?!]+$/g, "")
    .trim()
  return query || null
}

function volumeAction(text: string): { direction: VolumeDirection; steps: number; target: "system" | "spotify" } | null {
  ...
  if (/\b(increase|inc|raise|turn up|up|louder|boost)\b/.test(t)) return { direction: "up", steps: big ? 5 : (target === "spotify" ? 3 : 2), target }
  if (/\b(decrease|dec|lower|turn down|down|quieter|softer|reduce)\b/.test(t)) return { direction: "down", steps: big ? 5 : (target === "spotify" ? 3 : 2), target }
  return null
}
...
function whatsAppMessageRequest(text: string): { recipient: string; message: string } | null {
  ...
  const cleaned = text
    ...
    .replace(/\b(open|launch|start)\s+whats\s*app\s*(?:and|to)?\s*/ig, "")
    .trim()
  const sendMatch = cleaned.match(/\bsend\s+["']?(.+?)["']?\s+to\s+(.+?)(?:\s+(?:on|in)\s+whats\s*app\b|\s*$)/i)
  if (!sendMatch) return null
  const message = sendMatch[1]?.replace(/[.?!]+$/g, "").trim()
  const recipient = sendMatch[2]?.replace(/\b(on|in)\s+whats\s*app\b/ig, "").replace(/[.?!]+$/g, "").trim()
...
export async function* agentPipeline(
  ...
  if (volume) {
    ...
    const result = check.ok
      ? await hooks.onPostToolUse(toolName, spotify
    ...
    yield { type: "agent_text", text: failed ? "I could not adjust the volume." : (spotify ? "Spotify volume adjusted." : "Volume adjusted.") }
  ...
  if (spotifyQuery) {
    ...
    yield {
      type: "agent_text",
      text: failed ? `I could not play ${spotifyQuery} on Spotify.` : `Playing ${spotifyQuery} on Spotify.`,
    }
  ...
  if (whatsAppMessage) {
    ...
    const result = check.ok
      ? await hooks.onPostToolUse("send_whatsapp_message", await sendWhatsAppMessage(whatsAppMessage.recipient, whatsAppMessage.message))
      : `[DENIED: ${check.reason}]`
  ...
  const tools = applyHooks({ ...createAgentTools({ screenshotB64: req.screenshot_b64 }), ...mcpTools })
