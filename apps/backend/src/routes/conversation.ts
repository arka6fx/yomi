...
conversationRouter.post("/shared/turn", authenticate, requireConsent("conversation_history"), async (c) => {
  ...
  const assistantText = typeof body?.assistantText === "string" ? body.assistantText.trim().slice(0, MAX_APPEND_CHARS) : ""
  if (!userText || !assistantText) return c.json({ error: "userText and assistantText are required" }, 400)
