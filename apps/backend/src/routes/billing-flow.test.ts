...
describe("E2E: explore -> pro -> consume -> buy credits -> consume -> edge cases", () => {
  ...
  it("3: subscription.active webhook upgrades user to Pro and grants 300 credits", async () => {
    expect(flow.user.plan).toBe("explore")
    ...
    // 300 credits granted from Pro subscription
    const balance = flow.creditGrants
      .filter((g) => g.status === "active")
      .reduce((s, g) => s + g.creditsRemaining, 0)
    ...
    expect(proGrant!.creditsGranted).toBe(300)
  })

  it("4: consumes all 300 Pro credits, then blocks with credits_exhausted", async () => {
    // Pure-credit gating: 300 credits = 300 chats (1 credit each), then blocked.
    ...
    // After 300 chats, all credits consumed.
