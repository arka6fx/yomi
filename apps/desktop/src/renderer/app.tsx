...
function ThemeProvider({ children }: { children: React.ReactNode }) {
  // Amber is the only theme. Apply it synchronously before first paint (a useEffect would
  // flash the unstyled default). setTheme stays a no-op so the context shape is unchanged.
  React.useState(() => {
    applyTheme(AMBER)
    return null
  })
  ...
  return (
    <ThemeCtx.Provider value={{ theme: AMBER, setTheme: () => {} }}>{children}</ThemeCtx.Provider>
  )
}
