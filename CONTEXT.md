...
**Capability**:
Permission for a caller to _attempt_ a class of operation, checked before execution —
it answers "may this caller even try?". Distinct from an _approval_, which answers "does
the user consent to this specific write?". A caller lacking the capability is denied
outright; a caller holding it may still hit the approval gate.
_Avoid_: permission, grant (reserve those for the user-facing act of granting one).

**Scope**:
The unit a capability is expressed as — a `resource:action` string like `memory:read`
or `agent:execute`. Wildcards widen it: `memory:*` (all actions on memory), `*:*`
(everything, reserved for `core` callers).
_Avoid_: permission string.

**Resource**:
The thing a caller interacts with — one of `memory`, `schedule`, `connector`, `agent`,
`profile`, `filesystem`. The left half of a scope.
_Avoid_: object, target.

**Action** (capability):
The verb in a scope — `read`, `write`, `delete`, or `execute`. The right half of a
scope. NOT `pendingActions.action`, which names a concrete connector operation (e.g.
`send_email`) — call _that_ an operation to keep the two apart.
_Avoid_: operation.

**Trust boundary**:
The tier a piece of code runs at, fixed by how it was loaded: `core` (built into
yomi-core, full trust — implicit `*:*`), `plugin` (registered, checked against its
manifest), or `external` (MCP/API caller, limited to the authenticated caller's scopes).
Determines how a caller's capability set is resolved.
_Avoid_: privilege level, tier, sandbox.

**Capability manifest**:
What a `plugin`-boundary caller declares it needs: `{ required: Scope[], optional:
Scope[] }`. A missing `required` scope means the plugin can't load; a missing `optional`
one means it degrades gracefully.
_Avoid_: permission list.

**Approval gate**:
The pre-existing, _separate_ check that queues a _pending action_ for user consent on a
risky write. Orthogonal to capabilities and applied second: a call clears the capability
check, then the approval gate. Failing the capability check denies immediately; tripping
the approval gate queues a pending action instead.
_Avoid_: capability check (they are different gates).
